...
const emptyVehicle = (slNo = 1): Vehicle => ({
  slNo,
  hsnCode: "8703.21.69",
  vehicleName: "",
  exteriorColour: "",
...
const EditOrder = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [clientsRes, orderRes] = await Promise.all([
          axios.get(`${apiConfig.baseURL}/clients`),
        ...
        setForm({
          ...
          portOfLoading: order.portOfLoading || "Any Port in India",
          portOfDischarge: order.portOfDischarge || "Any Port in Sri Lanka",
          paymentTerms: order.paymentTerms || "",
          buyerRef: order.buyerRef || "",
          date: order.date ? new Date(order.date).toISOString().split("T")[0] : new Date().toISOString().split("T")[0],
          bankName: order.bankName || "IDFC FIRST BANK",
          accountNo: order.accountNo || "10247939579",
          branch: order.branch || "KARVE NAGAR",
          ifscCode: order.ifscCode || "IDFB0041359",
        });
        ...
        const client = clientList.find((c: any) => c._id === (order.clientId?._id || order.clientId));
  ...
  const addVehicle = () => {
    setVehicles([...vehicles, emptyVehicle(vehicles.length + 1)]);
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      setLoading(true);
      await axios.put(`${apiConfig.baseURL}/orders/${id}`, buildPayload());
  ...
  const handleUpdateAndDownload = async () => {
    ...
    try {
      setLoading(true);
      await axios.put(`${apiConfig.baseURL}/orders/${id}`, buildPayload("Confirmed"));
  ...
  return (
    <div className="bg-white rounded-2xl shadow-md border border-gray-200 px-6 py-6 space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-xl font-semibold text-blue-600">Edit Order</h1>
          ...
          {selectedClient && (
            <>
              <div>
                ...
                <p className="bg-white border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-700">{selectedClient.companyName || "-"}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Country</p>
                <p className="bg-white border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-700">{selectedClient.country}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Contact</p>
                <p className="bg-white border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-700">{selectedClient.phone}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 mb-1">Email</p>
                <p className="bg-white border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-700">{selectedClient.email || "-"}</p>
              </div>
              <div className="md:col-span-2">
                <p className="text-xs text-gray-500 mb-1">Address</p>
                <p className="bg-white border border-gray-200 rounded-lg px-4 py-2.5 text-sm text-gray-700">{selectedClient.address || "-"}</p>
              </div>
            </>
          )}
        </div>
      ...
      <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <select name="incoterm" value={form.incoterm} onChange={handleFormChange}
              className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:outline-none">
          ...
          <div>
            ...
            <input type="text" name="portOfLoading" value={form.portOfLoading} onChange={handleFormChange}
              className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
          </div>

          <div>
            <label className="block text-sm mb-1">Port of Discharge</label>
            <input type="text" name="portOfDischarge" value={form.portOfDischarge} onChange={handleFormChange}
              className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
          </div>

          <div>
            <label className="block text-sm mb-1">Payment Terms</label>
            <input type="text" name="paymentTerms" value={form.paymentTerms} onChange={handleFormChange}
              className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
          </div>

          <div>
            <label className="block text-sm mb-1">Buyer's Reference</label>
            <input type="text" name="buyerRef" value={form.buyerRef} onChange={handleFormChange}
              className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
          </div>
          ...
          {vehicles.map((v, i) => (
            <div key={i} className="bg-white border border-gray-200 rounded-xl p-5">
              <div className="flex justify-between items-center mb-4">
                ...
                {vehicles.length > 1 && (
                  <button onClick={() => removeVehicle(i)} className="p-1.5 text-red-500 hover:bg-red-50 rounded">
                    <Trash2 size={16} />
                  </button>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div className="lg:col-span-2">
                  ...
                  <input type="text" value={v.vehicleName}
                ...
                <div>
                  ...
                  <input type="text" value={v.hsnCode}
                ...
                <div>
                  ...
                  <input type="text" value={v.exteriorColour}
                ...
                <div>
                  ...
                  <input type="text" value={v.chassisNo}
                ...
                <div>
                  ...
                  <input type="text" value={v.engineNo}
                ...
                <div>
                  ...
                  <input type="text" value={v.engineCapacity}
                ...
                <div>
                  ...
                  <input type="text" value={v.countryOfOrigin}
                ...
                <div>
                  ...
                  <input type="number" value={v.fobAmount}
                ...
                <div>
                  ...
                  <input type="number" value={v.freight}
                ...
                <div>
                  ...
                  <input type="number" value={v.quantity} min={1}
      ...
      <div className="bg-gray-50 rounded-xl p-6 border border-gray-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          ...
          <div>
            ...
            <input type="text" name="accountNo" value={form.accountNo} onChange={handleFormChange}
              className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
          </div>
          <div>
            <label className="block text-sm mb-1">Branch</label>
            <input type="text" name="branch" value={form.branch} onChange={handleFormChange}
              className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
          </div>
          <div>
            <label className="block text-sm mb-1">IFS Code</label>
            <input type="text" name="ifscCode" value={form.ifscCode} onChange={handleFormChange}
              className="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-blue-500 focus:outline-none" />
          </div>
        </div>
      </div>
