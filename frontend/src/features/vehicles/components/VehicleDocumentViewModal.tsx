...
const VehicleDocumentViewModal = ({ isOpen, onClose, vehicle }: Props) => {
  if (!isOpen) return null;

  // IMPROVED TOKEN RETRIEVAL
  // We check multiple common keys.
  // Please check your authStorage.ts to see which one is correct.
  const token =
  ...
  const getFileUrl = (field: string, download = false) => {
    // Ensure we don't have double slashes if apiConfig.baseURL ends in /
    const cleanBaseUrl = apiConfig.baseURL.endsWith("/")
      ? apiConfig.baseURL.slice(0, -1)
      : apiConfig.baseURL;

    const baseUrl = `${cleanBaseUrl}/vehicles/${vehicle._id}/files/${field}`;
    const params = new URLSearchParams();

    if (download) params.append("download", "true");

    // Only append if token exists
    if (token) {
...
export default VehicleDocumentViewModal;
