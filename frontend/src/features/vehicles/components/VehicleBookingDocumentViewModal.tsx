...
import { X, Eye, Download, FileText, AlertCircle, Receipt, Upload, Layers } from "lucide-react";
...
const VehicleBookingDocumentViewModal = ({ isOpen, onClose, booking }: Props) => {
  ...
  // Merged PDF endpoint — must return application/pdf (binary), NOT an HTML page
  const mergedPdfUrl = appendToken(
    `${apiConfig.baseURL.replace(/\/$/, "")}/vehicle-bookings/${booking._id}/client-documents/merged-pdf`,
  );

  // Download URL for merged PDF
  const mergedDownloadUrl = `${mergedPdfUrl}${mergedPdfUrl.includes("?") ? "&" : "?"}download=true`;
  ...
  const clientDocs = isClient
    ? [
        ...
        },
        {
          ...
          url: booking?.documents?.hblDocument ? getFileUrl("hblDocument", false) : "",
          Icon: FileText,
        },
        {
          label: "BV Certificate",
          available: !!booking?.documents?.bvCertificate,
          url: booking?.documents?.bvCertificate ? getFileUrl("bvCertificate", false) : "",
          Icon: FileText,
        },
        {
          label: "Shipping Bill",
          available: !!booking?.documents?.shippingBill,
          url: booking?.documents?.shippingBill ? getFileUrl("shippingBill", false) : "",
          Icon: FileText,
        },
      ]
    : [];
            ...
            {hasMergeableDocs && (
              <div className="mb-4 rounded-2xl border border-indigo-100 bg-indigo-50 p-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <p className="text-xs font-black text-indigo-800">Merged Documents</p>
                    <p className="text-[9px] font-bold text-indigo-400 uppercase tracking-tighter">
                      Commercial Invoice · HBL · BV Certificate · Shipping Bill
                    </p>
                  </div>
                </div>
