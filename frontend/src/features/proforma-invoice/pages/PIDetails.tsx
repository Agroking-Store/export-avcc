...
const LCUploadModal = ({ piId, onClose, onSuccess }: LCModalProps) => {
  ...
  const startTicker = (from: number, to: number, label: string) => {
    ...
    tickerRef.current = setInterval(() => {
      cur += Math.random() * 1.5;
      if (cur >= to) { cur = to; clearInterval(tickerRef.current!); }
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      setProgress(92); setProgressLabel("Comparing with PI data...");
      await new Promise((r) => setTimeout(r, 300));
      setProgress(100); setProgressLabel("Done");

      if (!res.comparison) { toast.error("Unexpected server response"); return; }
  ...
  const reset = () => {
    setSelectedFile(null); setResult(null); setProgress(0); setProgressLabel("");
  ...
  const vehicleMismatches = result?.mismatches.filter((m) => m.type === "vehicle") ?? [];
  const generalMismatches = result?.mismatches.filter((m) => m.type !== "vehicle") ?? [];
  const vehicleMatches = result?.matchedFields.filter((m) => m.type === "vehicle") ?? [];
  const generalMatches = result?.matchedFields.filter((m) => m.type !== "vehicle") ?? [];
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-800 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-5 border-b border-zinc-100 dark:border-zinc-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-sm font-semibold text-zinc-900 dark:text-white">Verify Letter of Credit</h2>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Upload the LC PDF to check it against this PI</p>
            </div>
          </div>
              ...
              >
                ...
                {selectedFile ? (
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-medium text-zinc-900 dark:text-white truncate max-w-xs mx-auto">{selectedFile.name}</p>
                    <p className="text-xs text-zinc-500">{(selectedFile.size / 1024).toFixed(1)} KB · PDF</p>
                    {!processing && (
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">Click to select LC PDF</p>
              ...
              {processing && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <p className="text-xs text-zinc-500 dark:text-zinc-400">{progressLabel}</p>
                    <p className="text-xs font-mono font-medium text-zinc-700 dark:text-zinc-300">{progress}%</p>
                  </div>
            ...
            <div className="space-y-4">
              ...
              {result.wrongLCAttached && (
                <div className="flex items-start gap-3 px-4 py-3 rounded-xl border border-orange-300 dark:border-orange-500/40 bg-orange-50 dark:bg-orange-500/10">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-orange-700 dark:text-orange-400">Wrong LC Attached</p>
                    <p className="text-xs text-orange-600/80 dark:text-orange-500 mt-0.5">
                      None of the chassis numbers in this LC match the vehicles on this Proforma Invoice.
                      Please verify you have uploaded the correct LC document.
                    </p>
                  </div>
                </div>
              )}
              ...
              <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border
                ...
                <div>
                  <p className={`text-sm font-semibold ${isPassed ? "text-emerald-700 dark:text-emerald-400" : "text-red-700 dark:text-red-400"}`}>
                    {isPassed ? "All fields verified — LC matches PI" : `${result.mismatches.length} mismatch${result.mismatches.length > 1 ? "es" : ""} found`}
                  </p>
                  <p className={`text-xs mt-0.5 ${isPassed ? "text-emerald-600/80 dark:text-emerald-500" : "text-red-600/80 dark:text-red-500"}`}>
                    {isPassed
                ...
                <div className="space-y-2">
                  ...
                  <div className="space-y-1">
                    {vehicleMatches.map((m, i) => (
                      <div key={i} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20">
                        <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400 shrink-0" />
                        <div className="min-w-0">
                          <p className="text-xs font-medium text-emerald-700 dark:text-emerald-400">{m.field}</p>
                          <p className="text-[11px] text-emerald-600/80 dark:text-emerald-500 font-mono truncate">{m.piValue}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {generalMatches.length > 0 && (
                <div className="space-y-2">
                  <p className="text-xs font-semibold uppercase tracking-widest text-zinc-400 dark:text-zinc-500">Matched</p>
                  <div className="flex flex-wrap gap-2">
                    {generalMatches.map((m, i) => (
                      <span key={i}
                        ...
                        <CheckCircle2 className="w-3 h-3" />{m.field}
                      </span>
                    ))}
                  </div>
                </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-zinc-100 dark:border-zinc-800 bg-zinc-50/50 dark:bg-zinc-900/50">
          {result ? (
            <>
              <button onClick={reset} className="text-sm text-zinc-500 hover:text-zinc-700 dark:hover:text-zinc-300 transition-colors">
                Upload another
              </button>
...
const MismatchCard = ({ item, accent = "red" }: { item: CompareField; accent?: "red" | "orange" }) => {
  const colors = {
    red: {
      wrap: "border-red-200 dark:border-red-500/30",
      header: "bg-red-50 dark:bg-red-500/10 border-red-200 dark:border-red-500/20",
      icon: "text-red-500",
      title: "text-red-700 dark:text-red-400",
    },
    orange: {
      wrap: "border-orange-200 dark:border-orange-500/30",
      header: "bg-orange-50 dark:bg-orange-500/10 border-orange-200 dark:border-orange-500/20",
  ...
  return (
    <div className={`rounded-lg border ${colors.wrap} bg-white dark:bg-zinc-900 overflow-hidden`}>
      ...
      <div className="grid grid-cols-2 divide-x divide-zinc-100 dark:divide-zinc-800">
        <div className="px-3 py-2">
          <p className="text-[10px] font-medium uppercase tracking-widest text-zinc-400 mb-1">PI Value</p>
          <p className="text-xs text-zinc-700 dark:text-zinc-300 break-all font-mono">
            {item.piValue || <span className="italic not-italic text-zinc-400">—</span>}
          </p>
        </div>
        <div className="px-3 py-2">
          <p className="text-[10px] font-medium uppercase tracking-widest text-zinc-400 mb-1">LC Value</p>
          <p className="text-xs text-zinc-700 dark:text-zinc-300 break-all font-mono">
            {item.lcValue || <span className="italic not-italic text-zinc-400">—</span>}
          </p>
        </div>
      </div>
    </div>
...
const PIDetails = () => {
  ...
  const getToken = () => {
    ...
    if (!token && localStorage.getItem("user")) {
      try { const u = JSON.parse(localStorage.getItem("user") || "{}"); token = u.token || u.accessToken; } catch { }
    }
  ...
  const getStatusColor = (s: string) => {
    switch (s?.toLowerCase()) {
      case "approved": return "bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-500/20";
      case "pending_approval": return "bg-amber-50 dark:bg-amber-500/10 text-amber-700 dark:text-amber-400 border-amber-200 dark:border-amber-500/20";
      case "lc_received": return "bg-blue-50 dark:bg-blue-500/10 text-blue-700 dark:text-blue-400 border-blue-200 dark:border-blue-500/20";
      case "expired": return "bg-red-50 dark:bg-red-500/10 text-red-700 dark:text-red-400 border-red-200 dark:border-red-500/20";
  ...
  const formatAddress = (addr: any) => {
    if (!addr) return null;
    if (typeof addr === "string") return <p className="text-sm text-zinc-600 dark:text-zinc-400">{addr}</p>;
    return (<>
      {addr.houseBuilding && <p className="text-sm text-zinc-600 dark:text-zinc-400">{addr.houseBuilding}</p>}
      {addr.streetArea && <p className="text-sm text-zinc-600 dark:text-zinc-400">{addr.streetArea}</p>}
      {addr.cityTown && <p className="text-sm text-zinc-600 dark:text-zinc-400">{addr.cityTown}</p>}
      <p className="text-sm text-zinc-600 dark:text-zinc-400">{[addr.state, addr.pincode].filter(Boolean).join(" - ")}</p>
    ...
    <div className="bg-[#FAFAFA] dark:bg-[#0A0A0A] p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 mb-8 pb-6 border-b border-zinc-200 dark:border-zinc-800">
          <div className="flex items-start gap-4">
            ...
            <div className="space-y-1">
              <div className="flex flex-wrap items-center gap-3">
                <h2 className="text-2xl font-bold tracking-tight text-zinc-900 dark:text-white">Proforma Invoice</h2>
                {pi && <span className={`px-2.5 py-0.5 text-[11px] font-bold uppercase tracking-wider rounded-full border ${getStatusColor(pi.status)}`}>{pi.status?.replace(/_/g, " ")}</span>}
              </div>
              <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm text-zinc-500 font-medium">
                ...
                {pi?.order_id && (
                  <p className="flex items-center gap-1.5 border-l border-zinc-300 dark:border-zinc-700 pl-4">
                    ...
                    <span className="font-mono text-zinc-900 dark:text-zinc-300">{typeof pi.order_id === "object" ? pi.order_id.orderId : pi.order_id}</span>
                  </p>
                )}
              </div>
            </div>
          ...
          <div className="flex items-center gap-2 self-end lg:self-center">
            ...
            <div className="flex items-center gap-2 ml-2">
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button variant="outline" size="sm" className="h-9 px-2 border-zinc-200"><FileDown className="w-4 h-4" /></Button>
                </DropdownMenuTrigger>
                <DropdownMenuContent align="end">
                  <DropdownMenuItem onClick={() => handlePdfAction("download")} className="cursor-pointer"><Download className="w-4 h-4 mr-2" /> Download PDF</DropdownMenuItem>
                  <DropdownMenuItem onClick={() => navigate(`/proforma-invoice/edit/${id}`)} className="cursor-pointer"><Edit className="w-4 h-4 mr-2" /> Edit Details</DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
            </div>
          </div>
        ...
        {loading && (
          <div className="flex justify-center items-center py-32">
            <div className="flex flex-col items-center gap-4 bg-white/50 dark:bg-zinc-900/50 p-8 rounded-2xl border border-zinc-100 dark:border-zinc-800">
              ...
              <p className="text-sm font-medium text-zinc-900 dark:text-white">Loading Document</p>
            </div>
          </div>
        )}
          ...
          <div className="bg-white dark:bg-[#0E0E10] border border-zinc-200 dark:border-zinc-800 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="p-8 sm:p-10 border-b border-zinc-100 dark:border-zinc-800/60 flex flex-col sm:flex-row justify-between gap-8 bg-zinc-50/50 dark:bg-zinc-900/20">
              <div className="flex-1">
                <h3 className="text-[11px] font-semibold uppercase tracking-widest text-zinc-500 dark:text-zinc-400 mb-3">Exporter Details</h3>
                {(pi.company_id as any)?.name ? (
                  <div className="space-y-1">
                    <p className="text-base font-medium text-zinc-900 dark:text-white">{(pi.company_id as any).name}</p>
                    {formatAddress((pi.company_id as any).address)}
                    {(pi.company_id as any).gstNumber && <p className="text-sm text-zinc-500 mt-2">GSTIN: <span className="font-mono">{(pi.company_id as any).gstNumber}</span></p>}
                  </div>
                ) : <p className="text-sm text-zinc-400 italic">Not specified</p>}
              </div>
              <div className="flex-1 sm:text-right">
                <h3 className="text-[11px] font-semibold uppercase tracking-widest text-zinc-500 dark:text-zinc-400 mb-3">Billed To</h3>
                <div>
                  <p className="font-medium text-zinc-900 dark:text-white">{(pi.client_id as any)?.name}</p>
                  <p className="text-sm text-zinc-600 dark:text-zinc-400">{(pi.client_id as any)?.companyName}</p>
                  {formatAddress((pi.client_id as any)?.address || (pi.client_id as any)?.country)}
                  <p className="text-sm text-zinc-500 mt-1">Code: <span className="font-mono">{typeof pi.client_id === "object" ? pi.client_id?.clientCode : ""}</span></p>
                </div>
              </div>
            </div>
            ...
            <div className="px-8 sm:px-10 py-6 border-b border-zinc-100 dark:border-zinc-800/60 grid grid-cols-2 md:grid-cols-3 gap-6">
              <div><p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-400 mb-1">Issue Date</p><p className="text-sm font-medium text-zinc-800 dark:text-zinc-200">{formatDate(pi.createdAt)}</p></div>
              <div><p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-400 mb-1">Validity Date</p><p className="text-sm font-medium text-zinc-800 dark:text-zinc-200">{formatDate(pi.validityDate)}</p></div>
              <div><p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-400 mb-1">Payment Terms</p><p className="text-sm font-medium text-zinc-800 dark:text-zinc-200">{pi.paymentTerms || "Not specified"}</p></div>
              <div className="col-span-2 md:col-span-3 grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-6 mt-2">
                <div><p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-400 mb-1">Terms of Delivery</p><p className="text-sm font-medium text-zinc-800 dark:text-zinc-200">{pi.termsOfDelivery || "Not specified"}</p></div>
                <div><p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-400 mb-1">Incoterm</p><p className="text-sm font-medium text-zinc-800 dark:text-zinc-200">{pi.incoterm || "Not specified"}</p></div>
                <div><p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-400 mb-1">Port of Loading</p><p className="text-sm font-medium text-zinc-800 dark:text-zinc-200">{pi.portOfLoading || "Not specified"}</p></div>
                <div><p className="text-[11px] font-semibold uppercase tracking-widest text-zinc-400 mb-1">Port of Discharge</p><p className="text-sm font-medium text-zinc-800 dark:text-zinc-200">{pi.portOfDischarge || "Not specified"}</p></div>
              </div>
            </div>
            ...
            {/* Bank Details */}
            {((pi.company_id as any)?.bankDetails?.bankName || (pi.company_id as any)?.bankDetails?.accountNo) && (
              <div className="px-8 sm:px-10 py-6 bg-zinc-50/50 dark:bg-[#121214] border-t border-zinc-100 dark:border-zinc-800/60 rounded-b-2xl">
                <h3 className="text-[11px] font-semibold uppercase tracking-widest text-zinc-500 dark:text-zinc-400 mb-3">Bank Details</h3>
                <div className="flex flex-wrap gap-x-8 gap-y-2 text-sm text-zinc-700 dark:text-zinc-300">
                  {(pi.company_id as any).bankDetails?.bankName && <p><span className="text-zinc-500 mr-2">Bank:</span><span className="font-medium">{(pi.company_id as any).bankDetails.bankName}</span></p>}
                  {(pi.company_id as any).bankDetails?.accountNo && <p><span className="text-zinc-500 mr-2">A/C No:</span><span className="font-mono font-medium">{(pi.company_id as any).bankDetails.accountNo}</span></p>}
                  {(pi.company_id as any).bankDetails?.branchIfsc && <p><span className="text-zinc-500 mr-2">Branch/IFSC:</span><span className="font-mono font-medium">{(pi.company_id as any).bankDetails.branchIfsc}</span></p>}
                </div>
              </div>
            )}
          </div>
      ...
      {showLCModal && id && (
        <LCUploadModal
          ...
          onSuccess={() => { setShowLCModal(false); fetchPI(); }}
        />
      )}
    </div>
  );
